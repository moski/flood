...
module Flood
  #
  # Parse the Torrent unique information including files and pieces
  #
  class MetaInfo < Flood::Base
    ...
    attr_accessor :downloadable_files, :pieces_list
    ...
    def set_downloadable_files
      ...
      #
      if multiple_files?
        set_multi_downloadable_files
      else
        add_file_to_downloadable_list(name: name, length: total_size, start: 0, finish: total_size - 1)
      end
    end

    def set_multi_downloadable_files
      files.inject(0) do |offset, file|
        add_file_to_downloadable_list(
          name: file['path'][0], length: file['length'], start:  offset, finish: offset + file['length'] - 1)
    ...
    def add_file_to_downloadable_list(params)
      @downloadable_files << DownloadableFile.new(params)
    end
    ...
    def set_pieces
      ...
      (0...number_of_pieces).each do |index|
        ...
        @pieces_list << Piece.new(
          index: index, start_byte: start_byte, end_byte: end_byte, piece_hash: piece_hash)
      end
    end
